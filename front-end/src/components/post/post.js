...
export default function Post(props) {
  ...
  useEffect(() => {
    (async () => {
      const result = await axios(serviceURL + "/posts/" + props.match.params.id);
      setPost(result?.data);
      console.log(`result`, result?.data);
      // setComment(result.data?.comments);
      // setLock(result.data?.lockComment);
      setLoading(false);
    })();
  }, [props]);
      ...
      {loading ? (
        <>
          ...
          <div className="d-flex justify-content-center">
            <ReactLoading type="spinningBubbles" color="black" height={300} width={100} />
          </div>
        </>
      ) : (
        <>
          <div className="container">
            ...
            <img
              className="mb-5"
              src={getStrapiMedia(post?.avatar)}
              alt=""
              style={{ width: "50%" }}
            />
          </div>
